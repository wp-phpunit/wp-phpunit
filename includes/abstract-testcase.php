...
abstract class WP_UnitTestCase_Base extends PHPUnit_Adapter_TestCase
{
    ...
    /** @var int[]|null */
    private $expected_php_error_severities;

    /** @var string|null */
    private $expected_php_error_message;

    /** @var string[] */
    private $caught_php_errors = [];

    /** @var bool */
    private $php_error_handler_installed = false;
    ...
    public static function set_up_before_class()
    {
        ...
        if (\defined('WP_RUN_CORE_TESTS') && WP_RUN_CORE_TESTS) {
            static::reset_post_types_for_core_tests();
            static::reset_taxonomies_for_core_tests();
            static::reset_post_statuses_for_core_tests();
        }
    ...
    private static function reset_post_types_for_core_tests(): void
    {
    ...
    private static function reset_taxonomies_for_core_tests(): void
    {
    ...
    private static function reset_post_statuses_for_core_tests(): void
    {
    ...
    public function expectPhpDeprecationMessage(string $message): void
    {
        $this->expectPhpError([ E_DEPRECATED ]);
        $this->expected_php_error_message = $message;
    }

    public function expectWarning(): void
    {
        $this->expectPhpError([ E_WARNING, E_USER_WARNING ]);
    }

    public function expectWarningMessage(string $message, string $description = ''): void
    {
        $this->expected_php_error_message = $message;
    }
    ...
    {
        $this->expectPhpError([ E_NOTICE, E_USER_NOTICE ]);
    }

    public function expectNoticeMessage(string $message, string $description = ''): void
    {
        $this->expected_php_error_message = $message;
    }
    ...
    {
        $this->expectPhpError([ E_WARNING, E_USER_WARNING, E_NOTICE, E_USER_NOTICE, E_USER_ERROR ]);
    }

    public function expectErrorMessage(string $message, string $description = ''): void
    {
        $this->expected_php_error_message = $message;
    }

    private function expectPhpError(array $severities): void
    {
        $this->expected_php_error_severities = $severities;
        $this->caught_php_errors             = [];

        if ($this->php_error_handler_installed) {
            return;
        }

        set_error_handler(
            function (int $severity, string $message): bool {
                if (null === $this->expected_php_error_severities || ! \in_array($severity, $this->expected_php_error_severities, true)) {
                    return false;
                }

                $this->caught_php_errors[] = $message;
                return true;
            },
        );
        $this->php_error_handler_installed = true;
    }

    private function verifyExpectedPhpError(): void
    {
        if (! $this->php_error_handler_installed) {
            return;
        }
        ...
        restore_error_handler();
        $this->php_error_handler_installed = false;

        $caught   = $this->caught_php_errors;
        $expected = $this->expected_php_error_message;

        $this->expected_php_error_severities = null;
        $this->expected_php_error_message    = null;
        $this->caught_php_errors             = [];

        $this->assertNotEmpty($caught, 'Failed to capture an expected PHP error.');
    ...
    protected function checkRequirements()
    {
        ...
        if (\is_callable(PHPUnit\Framework\TestCase::class, 'checkRequirements')) {
